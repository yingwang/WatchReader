...
- **Read aloud** — a foreground media service reads the book with the watch's TTS engine, sentence by sentence with Chinese/English detection, and keeps going with the screen off or after you leave the reader; the reader follows along and highlights the sentence. The main thread never waits on the engine: it is shut down on a thread of its own and nothing is asked of it before it reports ready, because a cold engine start on a watch takes seconds
